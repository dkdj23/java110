...
// => 의존 객체를 꼽은 setter에 이 애노테이션을 붙이면
...
//   꼭 String을 java.sql.Date으로 바꾸기 위해 커스텀 프로퍼티 에디터를 
//   장착하는 것처럼.
//
//=> @Autowired 애노테이션 처리기를 장착하는 방법
//   AutowiredAnnotationBeanPostProcessor 클래스의 객체를 등록하라!
//   그냥 등록만 하면 된다
//   그러며 스프링 IoC 컨테이너가 알아서 이 객체를 사용하여 @Autowired를 처리한다.
//   
// BeanPostProcessor ?
// => 스프링 IoC 컨테이너는 객체를 생성한 후 BeanPostProcessor 를 실행한다.
//    즉 있으면 실행하고 없으면 실행하지 않는다.
