...
//    또는 프로퍼티 값을 설정하기 위해 setter를 호출할 떄,
//    XML 설정 파일에 지정된 문자열을 해당 파라미터 타입에 맞게 
//    자동으로 형변환한다.
//    단   primitive 타입에 대해서만자동 형변환 한다.
//    예) String ==> byte/short/int/long/double/boolean/char
//
// => 그 외 타입에 대해 형변환 하려면 별도의 변환기를 장착해야 한다.
//    그 별도의 변환기를 "커스텀 프로퍼티 에디터(custom property editor) 라 부른다.
//
// 커스텀 프로퍼티 에디터 만들기
// => java.beans.PropertyEditor 인터페이스를 구현한다.
//   - 구현할 메서드가 너무 많기 때문에 보통 이 인터페이스를 직접 구현하기 보다는
//     이 인터페이스를 미리 구현한 추상 클래스를 상속 받아 만든다.
