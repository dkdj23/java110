...
language 속성
    - 기본 값 'java'
    - 스크립트릿, 포현식에 작성할 수 있는 프로그래밍 언어를 지정한다.
    - 원래는 다양한 언어를 사용할 수 있게 규칙을 정했지만
      현업에서는 "java"만 사용한다.
      - 그리고 규칙상으로도 "java" 값만 가질 수 있다.
contentType 속성
    - 다음 자바코드를 _jspService() 안에 생성한다.
       response.setContentType(속성값)
pageEncoding 속성
    - JSP 엔진에게 JSP 파일이 어떤 문자표를 사용하여 인코딩 했는지 알려준다.
import 속성
    - 서블릿 클래스에 import 코드를 추가한다.
    - 예1) import 속성에 콤마를 사용하여 여러 import 문장을 사용할 수 있다.
          import="java.net.*,java.util.*"
...
buffer 속성
    ...
    - 기본 값은 8KB이다.
    - 만약 버퍼 크기를 초과하면 autoFlush의 설정에 따라 처리된다.
      autoFlush="true"
