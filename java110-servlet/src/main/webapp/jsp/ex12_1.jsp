...
- 보관소에 저장된 객체를 꺼낼 때 사용한다.
- 문법
    &lt;jsp:useBean scope="보관소이름" id="보관할 때 사용한 이름" class="저장된 객체의 타입">
  scope: application, session, request, page
    ...
    - scope를 지정하지 않으면 기본이 "page" 이다.
    - scope를 지정하지 않으면 기본이 page다.
  class: 
    - 보관된 객체의 타입을 가리킨다.
    - fully-qualified class name 이다.
                즉, 패키지명을 포함한 클래스 이름이어야 한다.
      page의 import 속성에 패키지를 지정하는 것과 상관없이 반드시 패키지명을 지정해야 한다.
    - 보관소에서 꺼낸 객체를 저장하기 위한 변수의 타입으로 사용한다.
    - 만약 보관소에 지정된 이름의 객체가 없다면, 이 속성에 지정된 객체를 임의로 만들 것이다.
