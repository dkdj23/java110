/* 서블릿 배치 정보(for XML)  - XML 태그로 배치 정보 설정하기
 * => 서블릿의 배치 정보를 설정하는데는 두 가지 방법이 있다.
 *    1) 애노테이션으로 배치하기
 *       @WebServlet 애노테이션을 서빌릇 클래스에 선언한다.
 *    2) XML 태그로 배치하기
 *       /WEB-INF/web.xml 에 서블릿 배치 정보를 설정한다.
 *       'web.xml'을 배치설명자(Deployment Descriptor 파일; DD 파일) 라 부른다.
 *       
 * => 주의!
 *    web.xml 파일을 사용하여 서블릿을 배치할 때,  다음 속성 값을 false로 설정하지 않으면
