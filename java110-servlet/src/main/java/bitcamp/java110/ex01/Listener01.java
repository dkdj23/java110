// 리스너 만들기
// - 특정 상황에 놓이면 실행되는 객체이다. ( 특정 상황을 인지하고 감시 모니터링 할 떄)
...
// 리스너 만들었으면 서블릿 컨테이너에 등록해야 한다.
@WebListener
public class Listener01 implements ServletContextListener {
    public Listener01() {
        System.out.println("Listener01() 생성자 호출됨");
    }
    @Override
    public void contextInitialized(ServletContextEvent sce) {
        // 웹 애플리케이션이 시작될 때 이 메서드가 호출된다.
        // 이 웹 애플리케이션에 소속된 서블릿들이 공통으로 사용하는 자원은 이 메서드에서 준비한다.
    ...
    public void contextDestroyed(ServletContextEvent sce) {
        ...
        // contextInitialized() 에서 준비했던 자원을 해제한다.
