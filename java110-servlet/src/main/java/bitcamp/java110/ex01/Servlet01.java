...
// 2) 웹 애플리케이션의 경로를 web01(/web01)로 설정했을 경우
...
public class Servlet01 implements Servlet {
    ...
    public Servlet01() {
        System.out.println("Servlet01() 생성자 호출됨");
    }
    ...
    public void init(ServletConfig config) throws ServletException {
        ...
        // 예) DB 커넥션 준비하거나 외부 서버에 소켓을 연결하는 등의 작업이다.
    ...
    public ServletConfig getServletConfig() {
        ...
        // 그래서 이 메서드는 init() 에서 파라미터로 받은 ServletConfig 객체를 리턴해야 한다.
        return this.config;
    }
    ...
    public void service(ServletRequest req, ServletResponse res) throws ServletException, IOException {
        ...
        // 예를 들면 게시물 목록을 출력하거나 로그인을 처리하거나 회원 가입을 처리하는 등의 작업.
    ...
    주의!
    - 서블릿 객체는 클래스 당 한 개만 생성한다.
    - 요청할 때마다 생성되는 것이 아니다.
    - 클라이언트마다 객체가 생성되는 것이 아니라 한 객체를 사용하는 것이기 때문에
               객체의 인스턴스 변수를 공유하게 된다.
    - 클라이언트 마다 구분해서 값을 저장해야 한다면 서블릿의 인스턴스 변수를 사용해서는 안된다.
