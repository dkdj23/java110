...
public class Servlet06 extends GenericServlet {
    ...
    public void service(ServletRequest req, ServletResponse res) throws ServletException, IOException {
        ...
        // 주의! 
        // GET 요청에서 URL 에 한글이 포함된 경우 POST 처럼 따로 설정하지 않아도 된다.
        // 그런데 톰캣 7 이하에서는 server.xml에 설정을 해 줘야 한다.
...
// 7비트 장비?
// => 오래전에 구축된 일부 네트워크 장비(게이트웨이 등)가 7비트인 경우가 있다.
// => 이 경우 8비트 데이터를 전송하면 나올 때 1비트가 손실될 수 있다.

// 해결책?
// => 8비트 값을 ASCII 문자로 바꿔 보내고
// => 받은 쪽에서는 ASCII 문자를 원래의 바이틀 바꿔 사용한다.
// => "8비트 데이터를 ASCII 문자화 시키는 방법" = "URL 인코딩"
...
/* 그런데 왜 우리가 URL 인코딩/디코딩을 알아야 하는가?
   ...
   => 웹브라우저를 통해 서버에 데이터를 보내는 것이 아니라,
    ...
    URL 인코딩이 자동으로 처리되는 것이 아니기 때문에
     개발자가 직접 java.net.URLEncoder 를 사용하여 직접 URL 인코딩 해야 한다.
   => 웹브라우저가 보낸 데이터를 웹서버를 통해 받지 않고 직접 받는 경우가 있다.
   이런 경우에도 개발자가 java.net.URLDecoder 를 사용하여 직접 소켓 프로그래밍으로 URL 디코딩을 해야 한다.
      
/*

Request URL: https://search.naver.com/search.naver?sm=tab_hty.top&where=nexearch&query=UTF+%EC%9D%B8%EC%BD%94%EB%94%A9&oquery=%EB%84%A4%EC%9D%B4%EB%B2%84%EA%B8%88%EC%9C%B5&tqi=TIfg8spVuECssc5mhnZssssssHC-386249
