...
public class Test20 {
    ...
    @Autowired // sping 전용태그
    //@Resources // java 표준태그
    //@Inject?   //  가급전 자바 표준을 쓰는것을 권고 resources 나 inject
    ServletContext sc;
    ...
    // 방법1) DD 파일에 프론트 컨트롤러를 선언할 때 다음의 태그를 설정 한다. deployment descriptor file
    // <multipart-config>
    //    <max-file-size>2000000</max-file-size>
    // </multipart-config>
    //
    // 위 설정이 없는 경우 아래와 같은 에러가 난다.
    // org.springframework.web.util.NestedServletException: Request processing failed; 
    // nested exception is java.lang.IllegalStateException: Optional int parameter 'age' 
    // is present but cannot be translated into a null value due to being declared 
    // as a primitive type. Consider declaring it as object wrapper for the corresponding primitive type.
    //
