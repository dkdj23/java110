...
public class ServerApp implements Servlet {
    ...
    public void init(ServletConfig config) throws ServletException {
        // (생성자 다음에 호출됨)
        // 이 서블릿 객체를 생성한 후
        // 이 서블릿이 실행하는데 필요한 자원을 준비할 수 있도록 딱 한 번 호출한다. by 서블릿 컨테이너
        createIoCContainer();
        logBeansOfContainer();
    ...
    public void service(ServletRequest request, ServletResponse response) throws ServletException, IOException {
        ...
        // => HTTP 프로토콜 정보를 다루려면 request 객체를 원래의 타입으로 캐스팅 한 후 사용하라!
    ...
    @Override
    public void destroy() {
        // 서블릿 컨테이너는  서버를 종료하거나 웹 애플리케이션을 정지하기 바로 직전에
